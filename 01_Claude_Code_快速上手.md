...
```
我想從零建立一個 Vite + React + Tailwind 專案。
...
```
我現在在一個空目錄裡，想從零建立一個 Vite + React + Tailwind 專案。
...
```
先幫我把 React + Tailwind 專案建立好，並確認能啟動。
完成之後，我們再繼續寫貪食蛇遊戲的核心邏輯。
```
...
```
我要做一個經典的貪食蛇遊戲。
先給我一個核心邏輯的實作方案（例如蛇的資料結構、如何控制移動），告訴我準備修改哪些檔案、怎麼改。
先不要執行。
```
...
```
接下來，幫我加上隨機產生的蘋果。當蛇吃到蘋果時長度要增加，並且在畫面上方用 Tailwind 樣式做一個漂亮的計分板。
先說說你準備怎麼改。
```
...
- 給畫面上加一個按鈕
- 用 Tailwind 修改按鈕的顏色與圓角
- 找出某個報錯來自哪個檔案
- 給貪食蛇加上一個 Game Over 的重來畫面
- 讓它幫你解釋一個目錄結構
...
```
先看看這個專案的整體結構，告訴我主要元件在哪裡。

然後給我一個實作貪食蛇移動邏輯的方案，先別改程式碼。

確認方案後，再開始修改，並在最後確認專案是否能正常編譯。
```
