...
# 專案實戰：將 AI Agent 裝進口袋 —— OpenClaw 與 Telegram 串接

在之前的課程中，你已經掌握了 Claude Code 這個強大的開發工具。它是你在寫程式時最可靠的夥伴，但它通常只存在於你的開發環境中。

當你離開電腦、走在路上，或是需要一個「永遠在線」的 AI 助理幫你處理 Gmail、行事曆或是監控伺服器時，你需要的是 **OpenClaw (龍蝦)**。本章將帶你完成最後一塊拼圖：從「開發者工具」進化到「全天候個人 AI 助理」。

### 🤖 雙劍合璧：該選哪一個？

| 特性 | Claude Code (前五章重點) | OpenClaw (本章重點) |
| :--- | :--- | :--- |
| **主要定位** | 高階開發工具、寫程式、架構設計 | 私人管家、生活自動化、訊息助理 |
| **存在形式** | 終端機命令 (隨開隨用) | 伺服器服務 (24 小時在線) |
| **通訊方式** | 鍵盤輸入、CLI 介面 | Telegram / WhatsApp / Discord |
| **擅長任務** | 讀寫程式碼、Git 協作、執行腳本 | 串接 Gmail/Calendar、提醒、聯網查詢 |
